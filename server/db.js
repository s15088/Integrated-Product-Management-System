...
function createDefaultData(db) {
  ...
  const perms = [
    ...
    ['系统管理员','系统.用户管理',1],['系统管理员','系统.角色权限',1],['系统管理员','系统.审计查看',1],['系统管理员','系统.审计导出',1],['系统管理员','系统.数据归档',1],
    ...
    ['产品经理','系统.用户管理',0],['产品经理','系统.角色权限',0],['产品经理','系统.审计查看',1],['产品经理','系统.审计导出',1],['产品经理','系统.数据归档',1],
    ...
    ['开发工程师','系统.用户管理',0],['开发工程师','系统.角色权限',0],['开发工程师','系统.审计查看',0],['开发工程师','系统.审计导出',0],['开发工程师','系统.数据归档',0],
    ...
    ['开发负责人','系统.用户管理',0],['开发负责人','系统.角色权限',0],['开发负责人','系统.审计查看',0],['开发负责人','系统.审计导出',0],['开发负责人','系统.数据归档',0],
    ...
    ['测试负责人','系统.用户管理',0],['测试负责人','系统.角色权限',0],['测试负责人','系统.审计查看',0],['测试负责人','系统.审计导出',0],['测试负责人','系统.数据归档',0],
    ...
    ['普通成员','系统.用户管理',0],['普通成员','系统.角色权限',0],['普通成员','系统.审计查看',0],['普通成员','系统.审计导出',0],['普通成员','系统.数据归档',0],
  ];
...
async function initDB() {
  ...
  if (!existing.length) {
    ...
    // Migrate: ensure all required permissions exist
    migratePermissions(_db);
    saveToFile();
  }
...
// ============================================================
// Migrate permissions: ensure all required permissions exist
// Runs on every init for existing databases to add new permissions
// ============================================================
function migratePermissions(db) {
  ...
  for (const [role, key, allowed] of perms) {
    const existing = db.exec(
      `SELECT 1 FROM permission WHERE role = '${role}' AND permission_key = '${key}'`
    );
    if (!existing.length || existing[0].values.length === 0) {
      const pid = `${role}-${key}`.replace(/\./g, '-');
      db.run(
        'INSERT INTO permission (permission_id, role, permission_key, allowed) VALUES (?, ?, ?, ?)',
        [pid, role, key, allowed]
      );
      added++;
    }
  ...
  if (added > 0) {
    console.log(`Migrated ${added} new permission(s)`);
  }
}
